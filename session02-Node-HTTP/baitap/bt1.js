...
const server = http.createServer((req, res) => {
  ...
  const overviewPage = fs.readFileSync("./overview.html", "utf-8");
  const productPage = fs.readFileSync("./product.html", "utf-8");
  const cardTemplate = fs.readFileSync("./card-temple.html", "utf-8");
  const dataFruits = JSON.parse(fs.readFileSync("./data.json", "utf-8"));
  const searchBox = fs.readFileSync("./search.html", "utf-8");
  const createBox = fs.readFileSync("./create.html", "utf-8");
  ...
  if (pathname === "/") {
    ...
    const dataFruitsNew = dataFruits.filter((item) => item.id != idDelete);
    console.log(dataFruitsNew);
    // fs.writeFileSync("./data.json", JSON.stringify(dataFruitsNew));
